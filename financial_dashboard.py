...
# Function to import CSV data with detailed skip logging
    ...
    try:
        ...
        for index, row in df.iterrows():
            if (pd.isna(row['date']) or 
                ...
                skipped_details.append(f"Invalid data in row {index+2}: {row.to_dict()}")
                skipped += 1
                continue
            ...
            if existing:
                if abs(float(existing[0]) - float(row[expected_columns[-1]])) < 0.01:
                    skipped_details.append(f"Skipped duplicate in row {index+2}: {row.to_dict()}")
                    skipped += 1
                    continue
                else:
                    updated += 1
        ...
        if skipped > 0:
            message += f"\nSkipped details: {'; '.join(skipped_details)}"
        return True, message
...
def get_data(table='monthly_updates'):
    ...
    if df.empty:
        return pd.DataFrame()
    # Handle mixed date formats (YYYY-MM and YYYY-MM-DD)
...
# Function for baseline prediction (compound latest value at S&P 500 historical return)
def baseline_prediction(current_value, start_date, end_date=datetime(2042, 12, 31), annual_rate=0.07):
    years = (end_date.year - start_date.year) + (end_date.month - start_date.month) / 12
    return current_value * (1 + annual_rate) ** years

# Function for ML prediction using ARIMA
def ml_prediction(df_pivot, future_date=datetime(2042, 12, 31)):
    ...
    try:
        ...
        # Calculate steps to future_date
        ...
        predicted_value = forecast[-1] if len(forecast) > 0 else None
...
st.header("Month-to-Month Breakdown")
if not df.empty:
    ...
    # Calculate % change from Dec of previous year
...
st.header("Retirement Forecast (Monthly Progress)")
goal_amount = st.number_input("Enter retirement goal ($):", min_value=0.0, value=1000000.0, step=10000.0)
annual_rate = st.selectbox("S&P 500 Return Rate:", ["7% (Real)", "10% (Nominal)"], index=0)
...
# Monthly totals for Kim and Sean
if not df.empty:
    df['year'] = df['date'].dt.year
...
# Baseline and ML predictions
if not df_total.empty:
    baseline_pred = baseline_prediction(latest_sean_kim, current_date, datetime(2042, 12, 31), annual_rate=annual_rate_value)
    ml_pred, error = ml_prediction(df_pivot, datetime(2042, 12, 31))
    
    # Generate monthly forecasts for plotting
    ...
    fig.update_layout(xaxis_title='Month', yaxis_title='Balance ($)', xaxis_tickangle=-45)
    st.plotly_chart(fig, use_container_width=True)

# Goal Progress Tracker
st.header("Retirement Goal Progress")
if latest_sean_kim > 0:
    progress = min(latest_sean_kim / goal_amount, 1.0)
    goal_data = {
        ...
        'Progress (%)': [progress*100, (baseline_pred / goal_amount * 100), 
                         (ml_pred / goal_amount * 100) if ml_pred else 'N/A']
    }
    def format_value(x):
    ...
    st.write(f"Current Progress: {progress*100:.1f}% (${latest_sean_kim:,.2f} of ${goal_amount:,.2f})")
...
if len(df_total) > 1:
    st.header(f"Performance vs. S&P 500 Benchmark ({annual_rate})")
...
# 2025 Month-to-Month Progress
st.header("2025 Month-to-Month Progress")
if not df.empty:
    df_2025 = df[df['date'].dt.year == 2025].pivot_table(index=df['date'].dt.strftime('%Y-%m'), 
                                                         columns='person', values='contribution', 
                                                         aggfunc='sum', fill_value=0).reset_index()
    df_2025 = df_2025.rename(columns={'date': 'month'})
    df_2025['Sean_Kim'] = df_2025['Sean'] + df_2025['Kim']
    
    fig = go.Figure()
    fig.add_trace(go.Scatter(x=df_2025['month'], y=df_2025['Sean'], mode='lines+markers', name='Sean'))
    fig.add_trace(go.Scatter(x=df_2025['month'], y=df_2025['Kim'], mode='lines+markers', name='Kim'))
    fig.add_trace(go.Scatter(x=df_2025['month'], y=df_2025['Taylor'], mode='lines+markers', name='Taylor'))
    fig.add_trace(go.Scatter(x=df_2025['month'], y=df_2025['Sean_Kim'], mode='lines+markers', name='Sean + Kim'))
    fig.update_layout(xaxis_title='Month', yaxis_title='Balance ($)', xaxis_tickangle=-45)
    st.plotly_chart(fig, use_container_width=True)
else:
    st.write("No 2025 data available.")
